...
function DiscountGroupMaster() {
  ...
  const { api_fetchDiscoutGroupListMap,api_deleteDiscountByIdMap,api_searchDiscoutGroupListMap } = ApiList()
  const navigate = useNavigate()

  useSetTitle('Fee Head Type Master')

  const data = [
    { id: 1, fee_head_type: 10, discount_group: 10, is_annual: 'Yes', is_optional: 'No', is_latefee_applicable: 'No' },
    { id: 2, fee_head_type: 10, discount_group: 10, is_annual: 'No', is_optional: 'Yes', is_latefee_applicable: 'Yes' },
    { id: 3, fee_head_type: 10, discount_group: 10, is_annual: 'Yes', is_optional: 'No', is_latefee_applicable: 'Yes' },
    { id: 4, fee_head_type: 10, discount_group: 10, is_annual: 'No', is_optional: 'Yes', is_latefee_applicable: 'Yes' },
    { id: 5, fee_head_type: 10, discount_group: 10, is_annual: 'Yes', is_optional: 'No', is_latefee_applicable: 'No' },
  ]
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      Cell: ({ cell }) => (
        <div className="flex">
          <button
            onClick={() => navigate(`/discount-group-map-form/${cell.row.original.id}`)}
          ...
          {/* <button
            onClick={() => navigate(`/allmodules-dues/${cell.row.original.id}`)}
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteDiscountByIdMap, requestBody, ApiHeader())
  ...
  const fetchMasterList = (values=null) => {
    ...
    if(values===null){
      requestBody={}
      url = api_fetchDiscoutGroupListMap
    }else{
      requestBody = {
        search:values?.search,
      }
      url = api_searchDiscoutGroupListMap
    }
    setisLoading(true)
    AxiosInterceptors.post(url, requestBody, ApiHeader())
      .then(function (response) {
        console.log('view discount group map..', response?.data?.data)
        if (response?.data?.status) {
...
<div className="main-sub-div">
          ...
          <div className='big-text'>Discount Group Map List</div>
        ...
        <button className="bg-[#3b82f6] w-14 max-[320px]:w-8 h-10 rounded-r-[5px] flex items-center justify-center text-white" type="submit">
          ...
          >
            ...
            <div className='hiplus-div'>
              <button onClick={() => navigate('/discount-group-map-form')}
                type="button"
      ...
      <Tabs listRoute={'/discount-group-map'} formRoute={'/discount-group-map-form'} />

      <div className='mt-6'>
        <span className='lists'> Discount Group Map List</span>
      </div>
...
export default DiscountGroupMaster