...
function FeeHeaderTypeMaster() {
  ...
  const { api_fetcHeadTypeList, api_deleteHeadTypeById,api_searchHeadTypeList } = ApiList()
  ...
  const COLUMNS = [
    ...
    // {
    //   Header: "Is Late-Fee Applicable?",
    //   accessor: "is_latefee_applicable",
    //   Cell: ({ cell }) => (<span>{nullToNA(cell.row.original.is_latefee_applicable)}</span>)
    // },
    {
      Header: "Status",
...

    {
      ...
      Cell: ({ cell }) => (
        <div className="flex">
          <button
            onClick={() => navigate(`/feeheadtype-master-form/${cell.row.original?.id}`)}
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteHeadTypeById, requestBody, ApiHeader())
  ...
  const fetchMasterList = (values=null) => {
    ...
    if(values===null){
      requestBody={}
      url = api_fetcHeadTypeList
    }else{
      requestBody = {
        search:values?.search,
      }
      url = api_searchHeadTypeList
    }
    setisLoading(true)
    seterroState(false)
...
<div className="main-sub-div">
          ...
          <div className='big-text'>Fee Head Type Master List </div>
        ...
        <button className="bg-[#3b82f6] w-14 max-[320px]:w-8 h-10 rounded-r-[5px] flex items-center justify-center text-white" type="submit">
          ...
          >
            ...
            <div className='hiplus-div'>
              <button onClick={() => navigate('/feeheadtype-master-form')}
                type="button"
      ...
      <Tabs listRoute={'/feeheadtype-master'} formRoute={'/feeheadtype-master-form'} />

      <div className='mt-6'>
        <span className='lists'>Fee Head Type List ({readymadeListData[0]?.academic_year})</span>
      </div>
...
export default FeeHeaderTypeMaster