...
function DiscountGroupMaster() {
  ...
  const { api_fetchDiscoutGroupList, api_deleteDiscountById,api_searchDiscoutGroupList } = ApiList()
  ...
  const COLUMNS = [
    ...
    //   Cell: ({ cell }) => (<span>{nullToNA(cell.row.original.discount_percent)}</span>)
    // },
    {
      Header: "Description",
    ...
    },
    {
      ...
      Cell: ({ cell }) => (
        <div className="flex">
          <button
            onClick={() => navigate(`/discount-master-form/${cell.row.original.id}`)}
...
<div className="main-sub-div">
          ...
          <div className='big-text'>Discount Group Master List</div>
        ...
        <button className="bg-[#3b82f6] w-14 max-[320px]:w-8 h-10 rounded-r-[5px] flex items-center justify-center text-white" type="submit">
          ...
          >
            ...
            <div className='hiplus-div'>
              <button onClick={() => navigate('/discount-master-form')}
                type="button"
      ...
      <Tabs listRoute={'/discount-master'} formRoute={'/discount-master-form'} />

      <div className='mt-6'>
        <span className='lists'>Discount Group List Master  ({readymadeListData[0]?.academic_year})</span>
      </div>
