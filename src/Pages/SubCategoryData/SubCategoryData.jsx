...
const MiscSubCategoryDemo = () => {
  ...
  const {
    ...
    api_fetchSubCategoryList,
    api_activesubcategory,
    api_searchSubCategoryList,
    api_postSubCategory,
    api_editSubCategory,
    api_getSubCategoryById
  } = ApiList();

  useSetTitle('Misc. Sub Category Master Master');
  ...
  const validationSchema = yup.object({
    miscCategoryId: yup.string().required('Select misc. category'),
    miscSubCategoryName: yup
      .string()
      .required(' Misc. sub category is required field')
  });
  ...
  const fetchMasterList = (values) => {
    ...
    if (values === undefined) {
      ...
      url = api_fetchSubCategoryList;
    } else {
      requestBody = {
        search: values
      };
      url = api_searchSubCategoryList;
    }
    setisLoading(true);
  ...
  const fetchCategoryMasterList = () => {
    setisLoading(true);
    AxiosInterceptors.post(api_activesubcategory, {}, ApiHeader())
      .then(function (response) {
  ...
  const COLUMNS = [
    ...
    {
      Header: 'Misc. Sub Category Name',
      accessor: 'misc_sub_category_name'
    },
    {
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(api_getSubCategoryById, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const sectionEditData = (data) => {
    ...
    formik.setFieldValue('miscCategoryId', data?.misc_category_id);
    formik.setFieldValue('miscSubCategoryName', data?.misc_sub_category_name);
  };
  ...
  const saveMasterForm = (values) => {
    ...
    let requestBody;
    let requestBodyBase = {
      miscCategoryId: values.miscCategoryId,
      miscSubCategoryName: values.miscSubCategoryName
    };
    if (currentId !== null) {
      url = api_editSubCategory;
      requestBody = requestBodyBase;
      requestBody.id = currentId;
    } else if (
      values?.miscSubCategoryName !== '' &&
      values?.miscCategoryId !== '' &&
      currentId == null
    ) {
      url = api_postSubCategory;
      requestBody = requestBodyBase;
    }
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteSubCategoryById, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
  ...
  const handleChange = (e) => {
    ...
    {
      name == 'miscSubCategoryName' &&
        formik.setFieldValue(
          'miscSubCategoryName',
          allowCharacterNumberSpaceInput(
            value,
            formik.values.miscSubCategoryName,
            50
          )
        );
    }
  ...
  const column = [
    ...
    { title: 'Misc Sub Category Name', feild: 'misc_sub_category_name' },
  ...
  const generatePdf = () => {
    const doc = new jsPDF();
    doc.text('Misc Sub Category Details', 10, 10);
    doc.autoTable({
      theme: 'grid',
      columns: column.map((col) => ({ ...col, dataKey: col.feild })),
      body: readymadeListData
    }),
      doc.save('MiscSubCategory.pdf');
  };
  return (
    <>
                    ...
                    >
                      <div className="grid grid-cols-12 mt-6 w-full">
                        ...
                        <div className="col-span-12 lg:col-span-8 w-full">
                          <select
                            {...formik.getFieldProps('miscCategoryId')}
                            type="text"
                            ...
                            placeholder="Select misc. category id"
                          >
                            <option value="">Select</option>
                            {categoryDataList?.map((data, index) => (
                              <option
                                selected={
                                  data?.misc_category_id ===
                                  formik.values.miscCategoryId
                                }
                                value={data?.id}
                              >
                                {data?.misc_category_name}
                              </option>
                            ))}
                          </select>
                          <br />
                          <span className="text-red-600  text-xs">
                            {formik.touched.miscCategoryId &&
                            formik.errors.miscCategoryId
                              ? formik.errors.miscCategoryId
                              : null}
                          </span>
                        </div>
                      ...
                      <div className="grid grid-cols-12 mt-6 w-full">
                        ...
                        <div className="col-span-12 lg:col-span-8 w-full">
                          <input
                            {...formik.getFieldProps('miscSubCategoryName')}
                            className="h-10  w-full lg:w-[60%] px-7 2xl:py-1.5 py-1 2xl:text-base text-sm font-normal text-gray-700  bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none placeholder-gray-300 shadow-md"
                            type="text"
                            placeholder="Enter misc. sub category name"
                          />
                          <br />
                          <span className="text-red-600  text-xs">
                            {formik.touched.miscSubCategoryName &&
                            formik.errors.miscSubCategoryName
                              ? formik.errors.miscSubCategoryName
                              : null}
                          </span>
                        </div>
...
export default MiscSubCategoryDemo;
