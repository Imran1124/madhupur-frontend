...
const RouteNameDemo = () => {
  ...
  const validationSchema = yup.object({
    routeName: yup.string().required("Route name is a required field"),
  });
      ...
      .then(function (response) {
        if (response?.data?.status === true) {
          if (response?.data?.data?.total === 0) {
            ...
            console.log("route name data", response?.data?.data?.data);
            setreadymadeListData(response?.data?.data?.data);
            setlastPage(response?.data?.data?.last_page);
            setlastData(response?.data?.data?.total);
            setBlockToast(false);
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(api_getRouteDataById, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const sectionEditData = (data) => {
    ...
    formik.setFieldValue("routeName", data?.route_name);
  };
  ...
  const saveMasterForm = (values) => {
    ...
    if (currentId !== null) {
      url = api_editRouteData;
      requestBody = requestBodyBase;
      requestBody.id = currentId;
    } else if (values?.routeName !== "" && currentId == null) {
      url = api_postRouteData;
      requestBody = requestBodyBase;
    }
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteRouteData, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
  ...
  const handleChange = (e) => {
    ...
    //       "routeName",
    //       allowCharacterInput(value, formik.values.routeName, 50)
    //     );
    // }
  };
  ...
  const column = [
    { title: "Route Name", feild: "route_name" },
