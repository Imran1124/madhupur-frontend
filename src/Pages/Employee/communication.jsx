...
export default function Communication(props) {
  ...
  const getcountryUser = (setState) => {
    ...
    AxiosInterceptors.post(api_getactiveCountryData, {}, ApiHeader())
      .then(function (response) {
        console.log('countryData', response?.data?.data);
  ...
  // fetching data from country function
  useEffect(() => {
  ...
  const validationSchema = yup.object({
    ...
    mothers_name: yup
      .string()
      .matches(CATEGORY_REGEX, 'Please Enter value between A-Z to a-z'),
    p_pincode: yup.string().matches(pinRegExp, 'Pincode is not valid'),
    c_pincode: yup.string().matches(pinRegExp, 'Pincode is not valid')
  });
