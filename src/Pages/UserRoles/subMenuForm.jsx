...
function MenuForm() {
  ...
  const {
    api_getMenuActiveData,
    api_postSubmenuData,
    api_editSubmenuData,
    api_getSubmenuDataById,
  } = ApiList();
  const navigate = useNavigate();
  const { id } = useParams();

  const validationSchema = yup.object({
    subMenuName: yup.string().required(" Sub Menu name is required feild"),
  });
  ...
  const saveMasterForm = (values) => {
    ...
    if (id !== undefined) {
      url = api_editSubmenuData;
      requestBody = requestBodyBase;
      requestBody.id = id;
    } else {
      url = api_postSubmenuData;
      requestBody = requestBodyBase;
    }
  ...
  const fetchEditData = () => {
    ...
    AxiosInterceptors.post(api_getSubmenuDataById, requestBody, ApiHeader())
      .then(function (response) {
        console.log("fetch edit data response..", response?.data?.data);
        if (response?.data?.status) {
          subMenuEditData(response?.data?.data);
        } else {
          activateBottomErrorCard(true, `${response?.data?.message}`);
        }
        setisLoading(false);
  ...
  const subMenuEditData = (data) => {
    console.log("existing property details in prop address...", data);
    formik.setFieldValue("userType", data?.menu_id);
    formik.setFieldValue("menuId", data?.menu_id);
    formik.setFieldValue("subMenuName", data?.sub_menu_name);
  };
  const menuTypeList = () => {
    AxiosInterceptors.post(api_getMenuActiveData, {}, ApiHeader())
      .then(function (response) {
        if (response?.data?.status) {
    ...
    let value = e.target.value;
    {
      name == "subMenuName" &&
        formik.setFieldValue(
          "subMenuName",
          allowCharacterInput(value, formik.values.subMenuName, 50)
        );
    }
  };
      ...
      <div className={`main-div`}>
        <div className="main-inner-div">
          ...
          <div className="main-sub-div">
            <div className="main-sub-inner-div">
              <div className="text-div">
                <div className="big-text">Sub Menu Master Form</div>
            ...
            {id === undefined ? (
              <div className="tab-div">
                <div className="add-button-master-div">
                  <button
                    onClick={() => navigate("/sub-menu-form")}
                    type="submit"
          ...
          {id !== undefined ? (
            ...
            <Tabs listRoute={"/sub-menu"} formRoute={"/sub-menu-form"} />
          )}
          <div className="details-div">
            <span className="detailes">Details of Sub Menu</span>
          </div>
              ...
              <div className="form">
                ...
                <div className="col-span-4 grid grid-cols-12">
                  ...
                  <div className=" mb-6 col-span-12 md:col-span-2 md:pr-4">
                    ...
                    <input
                      {...formik.getFieldProps("subMenuName")}
                      type="text"
                    ...
                    />
                    <span className="text-red-600 text-xs">
                      {formik.touched.subMenuName && formik.errors.subMenuName
                        ? formik.errors.subMenuName
                        : null}
                    </span>
                  </div>
                </div>
