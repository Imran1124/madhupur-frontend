...
const VehicleDemo = () => {
  ...
  const {
    ...
    api_searchPickupPointData,
    api_postPickupPointData,
    api_editPickupPointData,
    api_deletePickupPointData,
  } = ApiList();

  useSetTitle("Pick Point Master Master");
  ...
  const fetchMasterList = (values) => {
    ...
    if (values === undefined) {
      ...
      url = api_getPickupPointData;
    } else {
      requestBody = {
        search: values,
      };
      url = api_searchPickupPointData;
    }
    setisLoading(true);
      ...
      .then(function (response) {
        if (response?.data?.status === true) {
          if (
            ...
            console.log("Pick up point data", response?.data?.data?.data);
            setreadymadeListData(response?.data?.data?.data);
            setlastPage(response?.data?.data?.last_page);
            setlastData(response?.data?.data?.total);
            setBlockToast(false);
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      accessor: "pickup_point_name",
    },
    {
      Header: "Pickup Point Address",
      accessor: "pickup_point_address",
    },
    {
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deletePickupPointData, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
    ...
    }
    {
      name == "pickupPointAddress" &&
        formik.setFieldValue(
          ...
          allowCharacterNumberSpaceInput(
            value,
            formik.values.pickupPointAddress,
            50
          )
        );
    }
  ...
  const column = [
    { title: "Pickup Point Name", feild: "pickup_point_name" },
    { title: "Pickup Point Address", feild: "pickup_point_address" },
