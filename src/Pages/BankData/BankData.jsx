...
const BankData = () => {
  ...
  const validationSchema = yup.object({
    bankName: yup
      ...
      .required('Bank name is required feild')
  });
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(api_getBankDataById, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const sectionEditData = (data) => {
    ...
    formik.setFieldValue('bankName', data?.bank_name);
  };
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteBankData, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
    ...
    let value = e.target.value;
    {
      name == 'bankName' &&
        formik.setFieldValue(
          'bankName',
          allowCharacterInput(value, formik.values.bankName, 50)
        );
    }
  };
  ...
  const column = [
    { title: 'Bank Name', feild: 'bank_name' },
    ...
    <>
      <div className="w-full h-[76vh] lg:h-auto overflow-auto ">
        <div className="flex items-start justify-start flex-col mt-5 ml-5 ">
          <h1 className="text-4xl font-semibold text-gray-700">Bank Form</h1>
