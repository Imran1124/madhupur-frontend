...
const StateDemo = () => {
  ...
  const fetchMasterList = (values) => {
    ...
    if (values === undefined) {
      ...
      url = api_getStateData;
    } else {
      requestBody = {
        search: values
      };
      url = api_searchStateData;
    }
    setisLoading(true);
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(api_getStateDataById, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteStateData, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
    ...
    let value = e.target.value;
    {
      name == 'stateName' &&
        formik.setFieldValue(
          'stateName',
          allowCharacterInput(value, formik.values.stateName, 50)
        );
    }
  };
    ...
    <>
      <div className="w-full h-[76vh] lg:h-auto overflow-auto ">
        <div className="flex items-start justify-start flex-col mt-5 ml-5 ">
          <h1 className="text-4xl font-semibold text-gray-700">State Form</h1>
