...
const CountryDemo = () => {
  ...
  const {
    ...
    api_postCountryData,
    api_editCountryData,
    api_getCountryDataById,
    api_searchCountryData,
    api_deleteCountryData
  } = ApiList();

  useSetTitle("Country Master");
  ...
  const fetchMasterList = (values) => {
    ...
    if (values === undefined) {
      ...
      url = api_getCountryData;
    } else {
      requestBody = {
        search: values,
      };
      url = api_searchCountryData;
    }
    setisLoading(true);
    AxiosInterceptors.post(url, requestBody, ApiHeader())
      .then(function (response) {
        ...
        if (response?.data?.status===true) {
          if(response?.data?.data?.total===0 || response?.data?.data?.data.length===0){
              toast.error(`Data not Found`)
              setreadymadeListData(response?.data?.data?.data);
          setlastPage(response?.data?.data?.last_page);
          setlastData(response?.data?.data?.total);
          }else{
          console.log("Country data", response?.data?.data?.data);
          setreadymadeListData(response?.data?.data?.data);
          setlastPage(response?.data?.data?.last_page);
          setlastData(response?.data?.data?.total);
  ...
  const COLUMNS = [
    {
      Header: "Sl No.",
      Cell: ({ cell }) => <span> {(number - 1) * postperpage + cell.row.index + 1} </span>,
    },
    {
        Header: "Country",
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(api_getCountryDataById, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const saveMasterForm = (values) => {
    ...
    if (currentId !== null) {
      url = api_editCountryData;
      requestBody = requestBodyBase;
      requestBody.id = currentId;
    } else if (values?.countryName !== "" && currentId == null) {
      url = api_postCountryData;
      requestBody = requestBodyBase;
    }
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteCountryData, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
          requestBody?.status=="active" ? toast.success("Activated Successfully") : toast.error("Deactivated Successfully")
    ...
    let value = e.target.value;
    {
        name == "countryName" &&
          formik.setFieldValue(
            "countryName",
            allowCharacterInput(value, formik.values.countryName, 50)
          );
      }
  ...
  const generatePdf = () => {
    const doc = new jsPDF();
    doc.text("Country Details", 10, 10);
    doc.autoTable({
      theme: "grid",
