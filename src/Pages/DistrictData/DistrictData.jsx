...
const CityDemo = () => {
  ...
  const {
    api_getDistrictDataByID,
    api_getDistrictData,
    api_searchDistrictData,
    api_postDistrictData,
    api_editDistrictData,
    api_deleteDistrictData,
    api_getactiveCountryData,
    api_getactiveStateData
  } = ApiList();

  useSetTitle('District Master');
  ...
  const fetchMasterList = (values) => {
    ...
    if (values === undefined) {
      ...
      url = api_getDistrictData;
    } else {
      requestBody = {
        search: values
      };
      url = api_searchDistrictData;
    }
    setisLoading(true);
  ...
  const fetchCityHeadTypeList = (data) => {
    AxiosInterceptors.post(api_getactiveStateData, {}, ApiHeader())
      .then(function (response) {
        if (response?.data?.status) {
  ...
  useEffect(() => {
    fetchCityHeadTypeList(formik.values.countryId);
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(api_getDistrictDataByID, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const sectionEditData = (data) => {
    ...
    formik.setFieldValue('countryName', data?.country_name);
    formik.setFieldValue('stateName', data?.state_name);
    formik.setFieldValue('cityName', data?.city_name);
  };
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(api_deleteDistrictData, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
    ...
    let value = e.target.value;
    {
      name == 'cityName' &&
        formik.setFieldValue(
          'cityName',
          allowCharacterInput(value, formik.values.cityName, 50)
        );
    }
  };
  ...
  const column = [
    { title: 'Country', feild: 'country_name' },
    { title: 'State', feild: 'state_name' },
    { title: 'City', feild: 'city_name' },
    ...
    <>
      <div className="w-full h-[76vh] lg:h-auto overflow-auto ">
        <div className="flex items-start justify-start flex-col mt-5 ml-5 ">
          <h1 className="text-4xl font-semibold text-gray-700">City Form</h1>
                    ...
                    >
                      <div className="grid grid-cols-12 mt-6 w-full">
                        <div className=" col-span-12 lg:col-span-4 w-full">
                          <label
                            className=" flex items-start justify-start ml-2 text-gray-800 text-md w-full font-bold"
                            htmlFor=""
                          >
                            Country
                        ...
                        <div className="col-span-12 lg:col-span-8 w-full">
                          <select
                            ...
                            type="text"
                            className={`h-10  w-full lg:w-[80%] px-7 2xl:py-1.5 py-1 2xl:text-base text-sm font-normal text-gray-700  bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none placeholder-gray-300 shadow-md`}
                            placeholder="Select countery id"
                          >
