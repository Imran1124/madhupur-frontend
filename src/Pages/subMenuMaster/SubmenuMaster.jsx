...
const ExamTermDemo = () => {
  ...
  const {
    ...
    sub_menu_crud_store,
    sub_menu_crud_edit,
    sub_menu_crud_get_by_id,
    sub_menu_crud_get,
    sub_menu_crud_search,
    sub_menu_crud_delete
  } = ApiList();

  useSetTitle('Sub Menu Master');
  ...
  const validationSchema = yup.object({
    ...
    subMenuName: yup.string().required('Required'),
      ...
      .then(function (response) {
        if (response?.data?.status === true) {
          if (response?.data?.data?.total === 0) {
            ...
            console.log('sub menu data', response?.data?.data?.data);
            setreadymadeListData(response?.data?.data?.data);
            setlastPage(response?.data?.data?.last_page);
            setlastData(response?.data?.data?.total);
          }
  ...
  const fetchEditData = (getid) => {
    ...
    AxiosInterceptors.post(sub_menu_crud_get_by_id, requestBody, ApiHeader())
      .then(function (response) {
  ...
  const deleteItem = () => {
    ...
    AxiosInterceptors.post(sub_menu_crud_delete, requestBody, ApiHeader())
      .then(function (response) {
        if (response?.data?.status === true) {
          fetchMasterList();
  ...
  const column = [
    ...
    { title: 'Sub Menu Name', feild: 'sub_menu_name' },
  ...
  const generatePdf = () => {
    const doc = new jsPDF();
    doc.text('Sub Menu Masters Details', 10, 10);
    doc.autoTable({
      theme: 'grid',
      columns: column.map((col) => ({ ...col, dataKey: col.feild })),
      body: readymadeListData
    }),
      doc.save('SubMenuMasters.pdf');
  };
  useEffect(() => {
                        ...
                        <div className="px-10 gap-4">
                          ...
                          <div className="mt-4">
                            <TextField
                              ...
                              size="small"
                              error={
                                formik.touched.subMenuName &&
                                Boolean(formik.errors.subMenuName)
                              }
                              helperText={
                                formik.touched.subMenuName &&
