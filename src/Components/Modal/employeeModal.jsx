...
export default function subadminmodal(props) {
  ...
  const { api_searchEmployeeByIdList } = ApiList();
  ...
  const validationSchema = yup.object({
    empNo: yup.string().required('Employee Number is required feild')
  });
    ...
    AxiosInterceptors.post(url, requestBody, ApiHeader())
      .then(function (response) {
        ...
        if (response?.data?.status === true) {
          if (response?.data?.message === 'Employee Not Found') {
            localStorage.setItem(
              'employee number',
              response?.data?.data?.emp_no
            );
            navigate('/step-form');
          } else if (
            response?.data?.message === 'Employee ID Already Existing'
          ) {
            navigate(`/editstep-form/${response?.data?.data?.id}`);
            activateBottomErrorCard(true, 'Employee ID already existing.');
          }
        } else {
  ...
  const handleChange = (e) => {
    ...
    {
      name == 'empNo' && formik.setFieldValue('empNo', value.trim());
    }
  };
  return (
    <>
